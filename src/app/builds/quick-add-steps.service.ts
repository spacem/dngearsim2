...
@Injectable()
export class QuickAddStepsService {
  ...
  steps = {
    exchangeStep: {
      ...
      getOptions: function (category, build, datas) {
        ...
        for (var e = 0; e < category.limitExchange.length; ++e) {
          ...
          var exchange = this.dntService.find('exchange.lzjson', 'ExchangeType', exId);
          var exName = '';
          if (exchange && exchange.length > 0 && exchange[0].NameID > 0) {
            exName = this.translationService.translate(exchange[0].NameID).toLowerCase();
          }
    ...
    accExchangeStep: {
      ...
      getOptions: function (category, build, datas) {
        ...
        for (var e = 0; e < category.limitExchange.length; ++e) {
          ...
          var exchange = this.dntService.find('exchange.lzjson', 'ExchangeType', exId);
          if (exchange && exchange.length > 0 && exchange[0].NameID > 0) {
            var exName = this.translationService.translate(exchange[0].NameID).toLowerCase();
    ...
    rankStep: {
      ...
      getOptions: function (category, build, datas) {
        ...
        var values = [];
        for (var id in this.valuesService.rankNames) {
          values.push(this.valuesService.rankNames[id]);
        }
        return values;
      },
      matchesItem: function (id, item) {
    ...
    titleStep: {
      name: 'select',
      getOptions: function (category, build, datas) {
        var allTitles = this.quickAddHelperService.findData(category, build, datas, 9999);
        var usefulTitles = [];
    ...
    enhanceEqStep: {
      ...
      getOptions: function (category, build, datas) {
        ...
        var item = this.quickAddHelperService.getItem(datas);
    ...
    itemStep: {
      name: 'select',
      getOptions: function (category, build, datas) {
        return this.quickAddHelperService.findData(category, build, datas);
      },
      isItemStep: true,
      hideName: true,
    },
    ...
    techSkillStep: {
      name: 'skill',
      getOptions: function (category, build, datas) {
        var items = this.quickAddHelperService.findData(category, build, datas, 1);
    ...
    itemNameStep: {
      name: 'item',
      getOptions: function (category, build, datas) {
        var items = this.quickAddHelperService.findData(category, build, datas);
        var itemNames = {};
        var itemNamesList = [];
    ...
    numStatsStep: {
      ...
      getOptions: function (category, build, datas) {
        ...
        for (var i = 0; i < items.length; ++i) {
          ...
          for (var j = 0; j < items[i].stats.length; ++j) {
            var stat = this.valuesService.stats[items[i].stats[j].id];
            if (stat && !stat.hide) {
              len++;
            }
          }
      ...
      matchesItem: function (id, item) {
        ...
        for (var j = 0; j < item.stats.length; ++j) {
          var stat = this.valuesService.stats[item.stats[j].id];
          if (stat && !stat.hide) {
            len++;
          }
        }
    ...
    highStatStep: {
      ...
      matchesItem: function (id, item) {
        ...
        for (var j = 0; j < item.stats.length; ++j) {
          ...
          var stat = this.valuesService.stats[val.id];
          if (stat.quickHigh) {
            if (val.id == id) {
              selectedVal += val.max;
            }
    ...
    customStep: {
      name: 'misc',
      getOptions: function (category, build, datas) {
        return this.valuesService.customItems;
      },
      isItemStep: true,
    },
  }
