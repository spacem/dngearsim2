...
function toOneDec(s: Stat) {
  return Math.round(s.max * 10) / 10;
}

function toNoDec(s: Stat) {
  return Math.floor(s.max);
}

function inThousands(s: Stat) {
  var val = Number(s.max);
  if (val < 100) {
    return val;
  }
  else if (val < 100000) {
...
export class ValuesService {
  ...
  getStats(data) {
    ...
    for (; ;) {
      ...
      if (stateId == 107) {
        ...
        var currentData = { id: stateId } as Stat;
  ...
  setupStat(stat: Stat, id: any) {
    stat.id = Number(id);
  }

  mergeStats(stats1: Stat[], stats2?: Stat[]) {
    var statMap: any = {};
