...
export class UiTranslationService {
  ...
  // this.translationService for each region go in here
  ...
  // this function gets called on language change to add the this.translationService to the main list
  uiTranslations() {
    return {
        addTranslations: function(region, data) {
            if(region && data && region in this.uiTranslationLookup) {
                var regionData = this.uiTranslationLookup[region];
